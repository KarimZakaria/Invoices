				...
				<div class="container-fluid">
					<div class="main-header-left ">
						...
						<div class="main-header-center mr-3 d-sm-none d-md-none d-lg-block">
							<input class="form-control" placeholder="ابحث عن أي شئ ..." type="search"> <button class="btn"><i class="fas fa-search d-none d-md-block"></i></button>
						</div>
					</div>
					<div class="main-header-right">
						...
						<div class="nav nav-item  navbar-nav-right ml-auto">
                                    ...
                                    <div class="dropdown-menu">
                                        <div class="menu-header-content bg-primary text-right">
                                            <div class="d-flex">
                                                ...
                                                <a class="badge badge-pill badge-warning mr-auto my-auto float-left" href="{{ route('mark') }}">
                                                    <span>تعيين قراءة الكل</span>
                                                </a>
                                            </div>
                                            <p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12">
                                                لديك <span id="notificationCount"> {{ auth()->user()->unreadNotifications->count() }} </span> اشعارات غير مقروءة
                                            </p>
                                        </div>
                                        <div class="main-notification-list Notification-scroll">
                                            <div id="unreadNotifications">
                                                @foreach(auth()->user()->unreadNotifications as $notification)
                                                    <a class="d-flex p-3 border-bottom" href="{{ route('Invoice_Details', $notification->data['id']) }}">
                                        ...
                                        <div class="dropdown-footer">
                                            <a href="{{ route('allNotifications') }}">رؤية كل الاشعارات</a>
                                        </div>
                                    </div>
                                </div>
                            @endcan
							...
							<div class="dropdown main-profile-menu nav nav-item nav-link">
								<a class="profile-user d-flex" href="{{ route('EditProfile', Auth::user()->id) }}"><img alt="" src="{{ asset('Profile/images/'. auth()->user()->image) }}"></a>
								<div class="dropdown-menu">
									<div class="main-header-profile bg-primary p-3">
										<a href="{{ route('EditProfile', Auth::user()->id) }}" class="d-flex wd-100p">
											<div class="main-img-user"><img alt="" src="{{asset('Profile/images/'. auth()->user()->image)}}" class=""></div>
											<div class="mr-3 my-auto">
												<h6>{{ Auth::user()->name }}</h6><span>{{auth()->user()->email}}</span>
											</div>
										</a>
									</div>
									<a class="dropdown-item" href="{{ route('UserProfile', Auth::user()->id) }}"><i class="bx bx-user-circle"></i>الملف الشخصي</a>
									<a class="dropdown-item" href="{{ route('EditProfile', Auth::user()->id) }}"><i class="bx bx-cog"></i> تعديل الملف الشخصي</a>
